...
public class Bot {
    ...
    public Command run() {
        ...
        /*
        if ((currentWorm.id == 1) || ((currentWorm.id == 2) && (currentWorm.bananaBombs.count==0)) || ((currentWorm.id == 3) && (currentWorm.snowballs.count==0))){
            enemyWorm = getFirstWormInRange();
        } else{
            enemyWorm = getFirstWormInRange2();
        }*/
        if (enemyWorm != null) {
            if ((currentWorm.id == 1) || ((currentWorm.id == 2) && (currentWorm.bananaBombs.count==0)) || ((currentWorm.id == 3) && (currentWorm.snowballs.count==0))){
...
/*
    private Worm getFirstWormInRange2() {
        Set<String> cells = constructFireDirectionLines(currentWorm.bananaBombs.range)
                .stream()
                .flatMap(Collection::stream)
                .map(cell -> String.format("%d_%d", cell.x, cell.y))
                .collect(Collectors.toSet());
